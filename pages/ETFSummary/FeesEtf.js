...
export default function FeesEtf(props) {
  ...
  const getETFStocks = async () => {
    ...
    setSymbol(response?.overview?.details?.symbol)
    setRatio(response?.fees?.expenses?.expenseRatio)
    setRatioIndex(response?.fees?.expenses?.expenseRatioIndex)
    setAsof(response?.asof)

    temp = []
    if (response?.fees?.fees?.feeindex > response?.fees?.fees?.feeself) {
      stacks = []
      stacks.push({
        value: parseFloat(getRoundOffValue(response?.fees?.fees?.feeindex)),
        color: '#E2433B',
        gradientColor: '#E2433B'
      })
      stacks.push({
        value: response?.fees?.fees?.feeindex / 3,
        color: 'transparent',
        ...
        innerBarComponent:() => {
          return (
            <View style={[globalStyle.justifyCenter, {height:'10%', alignItems:'flex-end'}]}>
              <Text style={{color:'#A1A9B6'}}>${parseFloat(getRoundOffValue(response?.fees?.fees?.feeindex))}</Text>
            </View>
          )
        }
      })
      ...
      stacks = []
      stacks.push({
        value: parseFloat(getRoundOffValue(response?.fees?.fees?.feeself)),
        color: '#2EBD85',
        gradientColor:'#2EBD85',
        innerBarComponent:() => {
          return (
            <View style={globalStyle.justifyCenter}>
              <Text style={{color:'#FFF'}}>${parseFloat(getRoundOffValue(response?.fees?.fees?.feeself))}</Text>
            </View>
          )
        }
      })
      stacks.push({
        value: parseFloat(getRoundOffValue(response?.fees?.fees?.feeindex - response?.fees?.fees?.feeself)),
        color: '#b3ffb3',
        gradientColor:'white'
      })
      stacks.push({
        value: response?.fees?.fees?.feeindex / 3+ 5,
        color: 'transparent',
        gradientColor:'transparent',
        innerBarComponent:() => {
          return (
            <View style={[globalStyle.justifyCenter, {height:'100%', alignItems:'flex-start'}]}>
              <Text style={{color:'#2EBD85'}}>You save ${parseFloat(getRoundOffValue(response?.fees?.fees?.feeindex - response?.fees?.fees?.feeself))}</Text>
            </View>
          )
        }
      })
      ...
      setMaxValue(response?.fees?.fees?.feeindex + response?.fees?.fees?.feeindex / 3)
    }else{
      stacks = []
      stacks.push({
        value: parseFloat(getRoundOffValue(response?.fees?.fees?.feeindex)),
        ...
        innerBarComponent:() => {
          return (
            <View style={globalStyle.justifyCenter}>
              <Text style={{color:'#FFF'}}>${getRoundOffValue(response?.fees?.fees?.feeindex)}</Text>
            </View>
          )
        }
      })
      stacks.push({
        value: parseFloat(getRoundOffValue(response?.fees?.fees?.feeself - response?.fees?.fees?.feeindex)),
        color: '#ff704d',
        gradientColor:'white'
      })
      stacks.push({
        value: response?.fees?.fees?.feeself / 3 + 5,
        color: 'transparent',
        gradientColor:'transparent',
        innerBarComponent:() => {
          return (
            <View style={[globalStyle.justifyCenter, {alignItems:'flex-start', height:'100%'}]}>
              <Text style={{color:'#E2433B'}}>You pay an extra ${getRoundOffValue(response?.fees?.fees?.feeself - response?.fees?.fees?.feeindex)}</Text>
            </View>
          )
        }
      })
      ...
      stacks.push({
        value: response?.fees?.fees?.feeself / 3,
        color: 'transparent',
        gradientColor:'transparent',
        marginBottom:5,
        innerBarComponent:() => {
          return (
            <View style={[globalStyle.justifyCenter, {height:'100%', alignItems:'flex-end'}]}>
              <Text style={{color:'#A1A9B6'}}>${getRoundOffValue(response?.fees?.fees?.feeself)}</Text>
            </View>
          )
        }
      })
      ...
      setMaxValue(response?.fees?.fees?.feeself + response?.fees?.fees?.feeself / 3)
    }
    ...
    step = (Dimensions.get("window").width - 30)
    let ratioTemp = parseFloat(getRoundOffValue(response?.fees?.expenses?.expenseRatio))
