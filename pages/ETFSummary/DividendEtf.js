...
export default function DividendEtf(props) {
  ...
  const getETFStocks = async () => {
    ...
    let dividendSelf = [];
    let dividendIndex = [];
    const da = response?.dividends?.growthDividend?.selfDividends.slice(0, 5)

    await response?.dividends?.growthDividend?.indexDividends.slice(0, 5).forEach((value, index) => {
      dividendIndex.push({
        value: value.amount,
        label: value.year,
        index:index,
    ...
    setDividend({
      amount:response?.dividends?.dividend?.dividendAmount,
      frequency:response?.dividends?.dividend?.frequency
    })

    setStrength({
      self:response?.dividends?.strenthDividendYield?.self,
      index:response?.dividends?.strenthDividendYield?.marketAverage
    })

    setSymbol(regionSymbol)

    let symbolPercent = 0
    let marketLengthFlag = response?.dividends?.strenthDividendYield?.self > response?.dividends?.strenthDividendYield?.marketAverage ? true:false
    
    if(marketLengthFlag){
      if(response?.dividends?.strenthDividendYield?.marketAverage == 0)
        symbolPercent = 100
      else
        symbolPercent = parseInt(response?.dividends?.strenthDividendYield?.marketAverage / response?.dividends?.strenthDividendYield?.self * 100)
    }else{
      if(response?.dividends?.strenthDividendYield?.self == 0)
        symbolPercent = 100
      else
        symbolPercent = parseInt(response?.dividends?.strenthDividendYield?.self / response?.dividends?.strenthDividendYield?.marketAverage * 100)
    }
      ...
      <View style={globalStyle.container}>
        ...
        <View style={globalStyle.justifyBetween}>
          <View>
            ...
            <View style={{ flexDirection: "row", gap: 50, color: "#FFF", fontSize: 24 }}>
        ...
        >
          <LineChart
            ...
            hideDataPoints
            initialSpacing={20}
            spacing={parseInt((Dimensions.get('window').width - 55) / 5)}
