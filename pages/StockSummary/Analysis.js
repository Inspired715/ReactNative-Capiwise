...
export default function Analysis(props) {
  ...
  const [statusLabelData, setStatusLabelData] = useState([])
  const [metricsData, setMetricsData] = useState({});
  const [minEpsValue, setMinEPSValue] = useState(0);
  const [maxPerformanceValue, setMaxPerformanceValue] = useState(0);
  ...
  const getEarningMetricsData = () => {
    ...
    response?.earnings?.earningsHisCurrYr.forEach((value) => {
    ...
    response?.performance?.earningsHistory.slice(0, 5).reverse().forEach((value, i) => {
        earningsData.push({
    ...
    response?.performance?.revenue_history.slice(0, 5).reverse().forEach((value, i) => {
    ...
    combinedArray.forEach(item => {
      unit = parseInt(getLargeNumberPatten(item.value))
      if(unit > defaultSize)
        defaultSize = unit
    })
    ...
    if(defaultSize > 0){
      earningsData.map(item => {
        item.value = parseFloat(item.value / (1000**defaultSize))
      })
