...
export default function Dividend(props) {
  ...
  const getDividendData = () => {
    ...
    let susLast = parseInt(response?.dividends?.divLastYr / response?.earnings?.earningsRetainedPastYr * 100);
    let susCur = parseInt(response?.dividends?.divCurrYr / response?.earnings?.earningsRetainedCurrYr * 100);
    let sus = {
    ...
    if(marketLengthFlag){
      if(response?.dividends?.annDivYieldIndustry == 0)
        ...
        symbolPercent = parseInt(response?.dividends?.annDivYield / response?.dividends?.annDivYieldIndustry * 100)
    }else{
      if(response?.dividends?.annDivYield == 0)
        symbolPercent = 100
      else
        symbolPercent = parseInt(response?.dividends?.annDivYieldIndustry / response?.dividends?.annDivYield * 100)
    }
      ...
      <View style={globalStyle.container}>
        ...
        <View style={globalStyle.justifyBetween}>
          <View>
            ...
            <View
              style={{ flexDirection: "row", gap: 50, color: "#FFF", fontSize: 24 }}
            >
        ...
        <View style={{flexDirection:'row', justifyContent:'space-evenly', marginTop:10}}>
          <View style={{ flexDirection:'column', gap:8, alignItems:'center' }}>
              <PieChart
                  ...
                  return <Text style={{fontSize: 24, color:'#FFF'}}>{sustainability.last[0].value}<Text style={{fontSize:20, color:'#FFF'}}>%</Text></Text>;
                  }}
              />
          ...
          <View style={{ flexDirection:'column', gap:8, alignItems:'center' }}>
              <PieChart
                  ...
                  centerLabelComponent={() => {
                    return <Text style={{fontSize: 24, color:'#FFF'}}>{sustainability.cur[0].value}<Text style={{fontSize:20, color:'#FFF'}}>%</Text></Text>;
                  }}
              />
