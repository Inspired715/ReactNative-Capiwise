  ...
  const getWatchList = async () => {
    ...
    await Api.getMyWatchList(token, mail)
      .then(async (response) => {
        ...
        setoriginalWatchList(response?.data)
      })
      .catch((error) => {
      });
      ...
      {isLoading ? <Loading /> :
        <>
          <View style={globalStyle.container}>
            <View style={[globalStyle.header, { alignItems: 'baseline' }]}>
          ...
          <View style={[styles.divider, { marginTop: 20, display: watchList.length > 0 ? "flex" : 'none' }]} />
          {watchList?.length > 0 ?
            <SymbolList data={watchList} />
            :originalWatchList.length > 0?
